...
BYTE PF_Read_ALL_SYS_PWRGD(void)
{
	if(Read_ALL_SYS_PWRGD())
	{
		return(1);
	}
	return(0);
...
{
	VOLMUTE_ON();    // 2014-10-28: Peter Modify for Audio
}

void PF_VOLMUTE_OFF(void)
{
	VOLMUTE_OFF();    // 2014-10-28: Peter Modify for Audio
}
...
BYTE PF_FAN_ON(void)
{
	...
	SET_MASK(ZTIER, BIT1);	// enable PWM output
	DCR0=160;		// enable to default fan speed on system boot
	DCR1=160;		// enable to default fan speed on system boot
	SET_MASK(GCR5, BIT1);		// select GPJ3 TACH1B as its alternative function
	DebugforceFan1RPM=0;

	SET_MASK(CLK6MSEL, BIT0); 	// set PWM clock source output as EC_Freq*2/3 = 24K
...
const sPowerSEQ code asPowerSEQS5S0[]=  
{
	...
	{ PF_Read_SLPS3, 		0,		1,  },
    { PF_DNBSWON_OFF,       0,      0,  },  
	{ PF_Read_ALL_SYS_PWRGD,0,      1,  },
	{PF_DGPU_ON,	100,	0},					// 2014-08-28: Peter add for VGA
		{PF_FAN_ON,		20,		0 }, 
   	{ PF_USB_ON, 		    5,		0,  },
...
const sPowerSEQ code asPowerSEQS0S5[]=  
{
    ...
    { PF_VOLMUTE_ON,  0,  0, },        // 2014-10-28: Peter add for Audio
    { PF_USB_OFF, 	        0,		0,  },	 
	{PF_DGPU_OFF,	0,	0}, 					// // 2014-08-28: Peter add for VGA
    { Hook_A20OFF,          0,		0,  },
	{ Hook_SMION, 		    0,		0,  },
	{ Hook_SCION, 		    0,		0,  },
    { PF_RVCC_OFF, 	        0,		0,  },	  
...
const sPowerSEQ code asPowerSEQS3S0[]= 
{
	...
	{ Hook_SCIOFF, 		    0,		0,  },
    { PF_DNBSWON_OFF, 		0,		0,  },	
//    	{PF_DGPU_ON,	0,	0},					// 2014-08-28: Peter add for VGA
    { PF_USB_ON, 		    5,		0,  },
	{ PF_WLan_Power_ON,     0,		0,  },
    { InitSio, 				10,     0,  },
	{ PF_ECPWROK_ON, 		0,      0,  },
...
//    FANCTL1_ON();
    ...
    KSOH1 |= 0x08;		// 2014-08-26: Peter add for Audio
}
...
void DoPowerStatus(void)
{
    ...
    presentstatus = (POWER_FLAG2&(SLP_S3_ON+SLP_S5_ON));
	...
	switch(POWER_FLAG2 & (SLP_S3_ON+SLP_S5_ON)) 
	{
		case 0: 	                        // goto S5
            switch (SysPowState) 
            {
		...
		case (SLP_S3_ON+SLP_S5_ON): // goto S0
            switch (SysPowState) 
            {
                case SYSTEM_S3 :
                    Oem_TriggerS3S0();
...
void Oem_SysPowerContrl(void)
{
    ...
    if((SystemIsS0)||(SystemIsS3)||(SystemIsS5))
    {
        CheckPowerState();
    }
